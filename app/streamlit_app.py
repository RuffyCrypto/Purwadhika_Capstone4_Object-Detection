# ===== DEBUG BLOCK INSERTED BY ASSISTANT =====
# This block shows whether Pillow (PIL) is available and lists installed packages in the environment.
import streamlit as _st_for_debug
import subprocess as _subproc, sys as _sys
try:
    with _st_for_debug.sidebar:
        _st_for_debug.write("### DEBUG Environment (inserted)")
        try:
            import PIL as _PIL
            _st_for_debug.write("PIL OK: " + getattr(_PIL, '__version__', 'unknown'))
        except Exception as _e_p:
            _st_for_debug.write("PIL import failed: " + str(_e_p))
        _proc = _subproc.run([_sys.executable, "-m", "pip", "list", "--disable-pip-version-check"], capture_output=True, text=True)
        _st_for_debug.write("pip list (first 40 lines):")
        for _line in _proc.stdout.splitlines()[:40]:
            _st_for_debug.write(_line)
except Exception:
    # if Streamlit isn't initialized yet or this block runs in a non-Streamlit context, ignore errors
    pass
# ===== END DEBUG BLOCK =====
