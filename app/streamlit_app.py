...
if run_btn:
    if uploaded is None:
        ...
        # build command using same interpreter
            ...
            # determine working dir (prefer running inside yolov12 if present)
            cwd = 'yolov12' if Path('yolov12').exists() else os.getcwd()
            # If cwd is 'yolov12', call detect.py directly; otherwise use the path under yolov12/
            if cwd == 'yolov12':
                detect_script = 'detect.py'
            else:
                detect_script = os.path.join('yolov12', 'detect.py')
            # use absolute paths to avoid confusion for weights/source
            cmd = [
                python_exec,
                detect_script,
            ...
            # robustly display stdout / stderr (works if proc.stdout is bytes or str)
def _safe_text(s):
    if s is None:
        return ""
...
try:
    ...
    st.write("Error while printing subprocess output:" , repr(_e))
    st.text("proc repr: " + repr(proc))
