...
# noninteractive + safer apt sequence with fallback for releaseinfo changes and retries
...
RUN set -eux; \
    ...
    apt-get install -y --no-install-recommends \
      ...
      gnupg \
      ffmpeg \
      libgl1-mesa-glx || apt-get install -y --no-install-recommends --fix-missing libgl1-mesa-glx ffmpeg curl ca-certificates; \
